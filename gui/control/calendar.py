...
class Calendar:
    ...
    def __place_widgets(s):
        ...
        xframe = ttk.Frame(s.G_Frame)
        hframe.pack(in_=s.G_Frame, side='top', pady=5, anchor='center')
        gframe.pack(in_=s.G_Frame, fill=tk.X, pady=5)
        bframe.pack(in_=s.G_Frame, pady=5)
        xframe.pack(in_=s.G_Frame, side='bottom', pady=5)
        ...
        # 时下拉框
        s.CB_hour = ttk.Combobox(bframe, width=3, values=['%02d' % hour for hour in range(0, 24)], validate='key',
                                 validatecommand=(Input_judgment_num, '%P'))
        s.CB_hour.current(0)
        s.CB_hour.grid(in_=bframe, column=0, row=0)
        s.CB_hour.bind('<KeyPress>', lambda event: s._update(event, True))
        s.CB_hour.bind("<<ComboboxSelected>>", s._update)
        tk.Label(bframe, text='时').grid(in_=bframe, column=1, row=0, padx=(0, 5))
        # 分下拉框
        s.CB_mins = ttk.Combobox(bframe, width=3, values=['%02d' % mins for mins in range(0, 60)], validate='key',
                                 validatecommand=(Input_judgment_num, '%P'))
        s.CB_mins.current(0)
        s.CB_mins.grid(in_=bframe, column=2, row=0)
        s.CB_mins.bind('<KeyPress>', lambda event: s._update(event, True))
        s.CB_mins.bind("<<ComboboxSelected>>", s._update)
        tk.Label(bframe, text='分').grid(column=3, row=0)
        # 秒下拉框
        s.CB_seconds = ttk.Combobox(bframe, width=3, values=['%02d' % secds for secds in range(0, 60)], validate='key',
                                    validatecommand=(Input_judgment_num, '%P'))
        s.CB_seconds.current(0)
        s.CB_seconds.grid(in_=bframe, column=4, row=0)
        s.CB_seconds.bind('<KeyPress>', lambda event: s._update(event, True))
        s.CB_seconds.bind("<<ComboboxSelected>>", s._update)
        tk.Label(bframe, text='秒', justify='left').grid(column=5, row=0)

        ttk.Button(xframe, text="确 定", width=6, command=lambda: s._exit(True)).grid(row=1, column=0, sticky='ns',
                                                                                    padx=20)
        ttk.Button(xframe, text="取 消", width=6, command=s._exit).grid(row=1, column=4, sticky='ne', padx=20)
    ...
    def _update(s, event=None, key=None):
        ...
        hour = int(s.CB_hour.get())
        mins = int(s.CB_mins.get())
        seconds = int(s.CB_seconds.get())
        if year == 0 or year > 9999: return
        s._canvas.place_forget()
        s._date = datetime(year, month, 1, hour, mins, seconds)
    ...
    def selection(s):
        ...
        hour = s._date.hour
        mins = s._date.minute
        seconds = s._date.second
        return str(datetime(year, month, int(s._selection[0]), hour, mins, seconds))
